...
class GameEngine {
  ...
  List<Tile> get paintOrderedBoardTiles => List<Tile>.from(boardTiles)
    ..sort((a, b) {
      ...
      return _tilePaintIndex(a).compareTo(_tilePaintIndex(b));
    });
  ...
  /// area. Tiles with a larger z-index/layer are above lower layers; same-layer
  /// ties use the source tile order that also drives `paintOrderedBoardTiles`,
  /// so the coverage test exactly matches what the player sees. This matches
  /// layered Mahjong/Triple Tile behavior where only fully uncovered, top-most
  /// visible tiles interact.
  ...
  bool _isAbove(Tile possibleCover, Tile tile) {
    ...
    return _tilePaintIndex(possibleCover) > _tilePaintIndex(tile);
  }

  int _tilePaintIndex(Tile tile) =>
      tiles.indexWhere((item) => item.id == tile.id);
