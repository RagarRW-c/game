...
void main() {
  ...
  test('same-layer tile painted above also blocks selection', () {
    final engine = GameEngine([
      tile(id: 'A_lower_same_layer', x: 0, y: 0, layer: 0),
      tile(id: 'B_upper_same_layer', x: 0.1, y: 0, layer: 0),
    ])..configureBoardGeometry(boardSize, fallbackTileSize);

    expect(
      engine.isTileCovered(engine.tileById('A_lower_same_layer')!),
      isTrue,
    );
    expect(
      engine.tapTile('A_lower_same_layer', boardSize, fallbackTileSize),
      isFalse,
    );
    expect(engine.isTileCovered(engine.tileById('B_upper_same_layer')!), isFalse);
  });
}
